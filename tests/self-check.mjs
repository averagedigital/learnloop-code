...
const codeEditorSource = readFileSync(new URL("../src/CodeEditor.jsx", import.meta.url), "utf8");
...
assert.match(appSource, /<CodeEditor/);
assert.match(codeEditorSource, /new EditorView/);
assert.match(codeEditorSource, /python\(\)/);
assert.match(codeEditorSource, /javascript\(\)/);
assert.match(codeEditorSource, /EditorView\.updateListener/);
...
assert.doesNotMatch(appSource, /composerMascot/);
...
assert.doesNotMatch(appStyles, /\.composerMascot/);
...
assert.doesNotMatch(mascotAssistantSource, /OpenHands|openAssistantUrl|mascotAssistantAgentFrame/);
...
assert.deepEqual(clampMascotSettings({ x: -100, y: 999, size: 400 }, { width: 390, height: 844, dialogOpen: false }), { x: 12, y: 642, size: 190, dialogWidth: 366, dialogHeight: 620 });
assert.deepEqual(clampMascotSettings({ x: 1300, y: -2, size: 100, dialogWidth: 900, dialogHeight: 900 }, { width: 1366, height: 768, dialogOpen: true }), { x: 1254, y: 12, size: 100, dialogWidth: 900, dialogHeight: 744 });
...
assert.match(mascotAssistantSource, /refreshDialog/);
assert.match(mascotAssistantSource, /ResizeObserver/);
assert.match(mascotAssistantSource, /dialogWidth/);
assert.match(mascotAssistantSource, /aria-label="Изменить размер маскота стрелками"[^>]*><svg/);
...
assert.match(appSource, /chats\.find\(\(chat\) => chat\.taskId === selected\?\.id && !isMascotChat\(chat\)\)/);
...
assert.match(serverSource, /mode: judge0Mode\(judgeUrl\)/);
...
assert.match(appSource, /className="taskBrief"/);
assert.match(appSource, /Условие задачи/);
assert.match(appSource, /assistantMarkdownToHtml\(log\?\.task\.prompt\)/);
...
assert.match(appStyles, /max-height: min\(80vh, 900px\)/);
...
assert.equal(JSON.parse(packageSource).version, "0.0.2");
assert.equal(JSON.parse(packageSource).license, "MIT");
...
assert.match(packageSource, /"codemirror"/);
assert.match(packageSource, /"@codemirror\/lang-python"/);
assert.match(packageSource, /"@codemirror\/lang-javascript"/);
assert.doesNotMatch(packageSource, /runtime:workspace|runtime:project|code-server|openhands/i);
...
assert.match(packageSource, /"runtime:default": "docker compose -f docker-compose\.workspace\.yml stop judge0-server judge0-worker judge0-db judge0-redis && docker compose -f docker-compose\.workspace\.yml up -d --build --remove-orphans falkordb graph-memory"/);
assert.match(packageSource, /"runtime:all": "docker compose -f docker-compose\.workspace\.yml up -d --build --remove-orphans falkordb graph-memory judge0-db judge0-redis judge0-server judge0-worker"/);
...
assert.doesNotMatch(envExampleSource, /CODE_SERVER|OPENHANDS|WORKSPACE_RUNTIME_URL|AGENT_RUNTIME_URL/);
...
assert.doesNotMatch(workspaceComposeSource, /code-server|openhands/i);
assert.match(workspaceComposeSource, /falkordb\/falkordb-server:latest/);
assert.match(workspaceComposeSource, /graph-memory/);
assert.match(workspaceComposeSource, /judge0\/judge0:1\.13\.1/);
assert.match(workspaceComposeSource, /judge0-server/);
...
assert.match(readmeSource, /assets\/screenshots\/tests\.png/);
assert.match(readmeSource, /assets\/screenshots\/task\.png/);
assert.match(readmeSource, /## Благодарности/);
assert.match(profileSource, /Публичный Judge0/);
assert.match(readmeSource, /Judge0/);
assert.match(readmeSource, /CodeMirror/);
assert.match(readmeSource, /FalkorDB/);
...
assert.doesNotMatch(serverSource, /code-server|openhands|WORKSPACE_RUNTIME_URL|AGENT_RUNTIME_URL/i);
...
assert.doesNotMatch(serverSource, /workspaceAgentFiles|workspaceAgentRun/);
